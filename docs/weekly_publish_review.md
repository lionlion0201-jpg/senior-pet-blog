...
1日2件(hook + cta)のままだと月$12程度。1件に絞れば月$6程度。

**2026-09-21の判断: 当面は1日2件のまま続ける。** 型別の効果を比較する材料を
作ることを優先した。各型20件に到達するのは2026-10-08の見込みで、そこで差が
出なければ安い方(1日1件)に寄せる。

**リンク無しで投稿した21件(9/21まで)は比較に混ぜないこと。** 流入を生まない
投稿と生む投稿を同じ土俵で比べても意味がない。
