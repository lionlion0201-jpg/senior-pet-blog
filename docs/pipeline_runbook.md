...
ディレクター ── Obsidian共通ルール+チェックリストで最終確認(承認 or 差し戻し)
    ...
    └─ 承認 → プロモーター(SNS素材作成、Pinterestは任意)
                ↓
        記事は非公開ドラフトとして保存(published: false)
                ↓
        週末に人間が公開レビュー(docs/weekly_publish_review.md)
                ↓
        承認分のみ公開・SNS投稿 → 次サイクルのアナリストへ
```

## 生成と公開の分離(2026年7月〜)

火・木・土の週3回、スケジュールタスクが記事を自動生成するが、**生成された記事は`published: false`の非公開ドラフトとして保存され、そのままではサイトに反映されない**。公開するかどうかの最終判断は、週末に`docs/weekly_publish_review.md`のチェックリストに沿って人間が行う。SNS(X)投稿も、記事が実際に公開された後に行う。
